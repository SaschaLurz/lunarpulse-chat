...
class SecureChat:
    ...
    def setup_ui(self):
        ...
        # Add file button
        self.file_button = tk.Button(self.input_frame, text="Send File", command=self.select_file)
    ...
    def setup_network(self):
        ...
        try:
            ...
            self.update_chat_history(f"[{timestamp}] System: Chat started on port {self.port}. Share this port number with your friend.")
        except OSError as e:
            self.update_status(f"Error: Could not bind to port {self.port}. {str(e)}")
            self.update_chat_history(f"[{timestamp}] System: Failed to start chat. Port {self.port} is unavailable.")
    ...
    def send_file(self, file_path):
        ...
        # Inform the user
        timestamp = self.get_timestamp()
        self.update_chat_history(f"[{timestamp}] System: Sending file '{file_name}' ({file_size} bytes)")
    ...
    def save_received_file(self, file_name):
        ...
        if save_path:
            ...
            try:
                ...
                timestamp = self.get_timestamp()
                self.update_chat_history(f"[{timestamp}] System: File '{file_name}' from {sender} saved to {save_path}")
            except Exception as e:
                timestamp = self.get_timestamp()
                self.update_chat_history(f"[{timestamp}] System: Error saving file: {str(e)}")
        ...
        while self.listening:
            try:
                ...
                try:
                    ...
                    if message["type"] == "message" and message["username"] != self.username:  
                        self.update_chat_history(f"[{message['timestamp']}] {message['username']}: {message['content']}")
                    
                    elif message["type"] == "join" and message["username"] != self.username:
                        self.update_chat_history(f"[{message['timestamp']}] System: {message['username']} has joined the chat.")
                    
                    # Handle file info message
                    elif message["type"] == "file_info" and message["username"] != self.username:
                        ...
                        self.update_chat_history(f"[{message['timestamp']}] System: {message['username']} is sending file '{file_name}' ({file_size} bytes)")
                    
                    # Handle file chunk message
                    elif message["type"] == "file_chunk" and message["username"] != self.username:
                        file_name = message["file_name"]
                        if file_name in self.incoming_files:
                            chunk_index = message["chunk_index"]
                            self.incoming_files[file_name]["chunks"][chunk_index] = message["data"]
                            self.incoming_files[file_name]["total_chunks"] = message["total_chunks"]
                            
                            # Check if we've received all chunks
                            if len(self.incoming_files[file_name]["chunks"]) == self.incoming_files[file_name]["total_chunks"]:
...
def main():
    ...
    port_input = input("Enter port number (e.g. 5555) or press Enter for automatic selection: ")
    if port_input.strip():
        port = int(port_input)
    else:
